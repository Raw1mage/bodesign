...
`--analyze` is required on a first run: without it, the scaffold errors when no analysis data exists. It runs the KiCad analysis scripts to produce the data the scaffold consumes. Omit it only when pre-generated analysis JSONs already exist in the analysis directory.
...
1. **Generate scaffold** — `kidoc_scaffold.py --analyze` auto-runs all available analyses, renders schematics, generates diagrams, and writes the markdown scaffold. The `--analyze` flag is required when no analysis data exists yet; without it the scaffold errors instead of running the analyzers.
...
```bash
python3 skills/kidoc/scripts/kidoc_scaffold.py \
  --project-dir . --spec my-report.json --analyze --output reports/custom.md
```
...
Additional `render_schematic`/`render_pcb` parameters (passed programmatically, not CLI flags): `highlight_nets=[...]`, `crop_refs=[...]`, `crop=(x,y,w,h)`, `mirror=True`, and `overlay='annotations.json'` (callout boxes with leader lines).

## Block Diagrams

```bash
python3 skills/kidoc/scripts/kidoc_diagrams.py --analysis analysis/ --output figures/
```

A single invocation emits all applicable figures — power tree, bus topology, and architecture block diagrams — for the supplied analysis JSON. There are no per-diagram selection flags; pass the optional `--emc`, `--thermal`, or `--spice` JSON paths to enable the corresponding charts, and `--force` to ignore the figure cache.
...
Run the `kicad` skill's analyzers first, then `emc` and `spice` if available. The scaffold auto-runs the `kicad` schematic/PCB analyses, `emc`, and `analyze_thermal.py` (the latter when both schematic and PCB JSONs are present) when source files are available, so manual pre-analysis is only needed for SPICE.

## References

Deep-dive documentation lives in `references/`:

| Reference | Covers |
|-----------|--------|
| `document-structure.md` | Scaffold markers, section types, and the GENERATED/NARRATIVE block model |
| `rendering-notes.md` | Schematic/PCB render options, layer presets, crop/focus/highlight behavior |
| `report-hdd.md` | Hardware Design Description structure and section guidance |
| `report-ce.md` | CE Technical File structure and compliance sections |
| `report-design-review.md` | Design Review Package structure |
| `report-icd.md` | Interface Control Document structure |
| `report-manufacturing.md` | Manufacturing Transfer Package structure |
