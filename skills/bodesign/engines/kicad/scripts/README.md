...
4. **Test against a regression harness**. Run new detectors across a broad set of real KiCad projects and assert stable output before merging.

5. **Count detections across the corpus** to calibrate sensitivity. A pattern that matches an implausibly large number of files suggests false positives; one that matches almost nothing might mean overly narrow keywords. Calibrate against a representative sample.
...
Detector work is validated against a regression harness that runs the analyzers over a large corpus of open-source KiCad projects spanning KiCad 5 through 10. The harness includes:

- Regression assertions over schematic, PCB, and gerber files
- Schema drift tests across all 8 analyzer types
- Equation audit (tagged equations), constants audit (switching frequencies), and bugfix guards

The harness is the authoritative validation layer; all new detector work validates against the corpus.
