...
def _get_digikey_token() -> tuple[str, str] | None:
    ...
    # Token cache (XDG-private dir, not predictable /tmp)
    cache_path = os.path.join(os.environ.get("XDG_RUNTIME_DIR") or os.environ.get("XDG_CACHE_HOME") or tempfile.gettempdir(), "digikey_token_cache.json")
    try:
