...
- `analyze_pcb.py --full`
- `cross_analysis.py`
- `<emc-skill-path>/scripts/analyze_emc.py`
...
Three additional CLIs consume analyzer JSON for verification, issue tracking, and fab gating:

```bash
# Cross-verify schematic design intent against the PCB implementation
python3 <skill-path>/scripts/cross_verify.py --schematic sch.json --pcb pcb.json [--thermal thermal.json] [--output out.json]

# Export analyzer findings to GitHub Issues via the gh CLI (dry-run unless --create)
python3 <skill-path>/scripts/export_issues.py analysis.json --repo owner/repo [--create] [--severity high] [--rule-id GP-001]

# Fabrication release gate — pass/fail readiness across all analyzers (--strict treats warnings as failures)
python3 <skill-path>/scripts/fab_release_gate.py --schematic sch.json --pcb pcb.json [--gerbers g.json] [--thermal t.json] [--emc emc.json] [--text] [--strict]
```
...
|-----------|-------|-------------|
| `schematic-analysis.md` | 1165 | Deep schematic review: datasheet validation, design patterns, error taxonomy, tolerance stacking, GPIO audit, motor control, battery life, supply chain |
...
| `report-generation.md` | 684 | Report template (critical findings at top), analyzer output field reference (schematic/PCB/gerber), severity definitions, writing principles, domain-specific focus areas, known analyzer limitations |
