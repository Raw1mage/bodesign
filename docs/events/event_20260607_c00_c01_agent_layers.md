...
- Re-baseline (handoff): reconciled the plan's status docs with committed runtime (C00/C01/C02 draft-grade runtime LANDED, 144 tests), synced `.state.json` proposed→implementing, amended the proposal scope to ratify the C01/C02 draft-emitter expansion, and recorded debts RB-1/RB-2/RB-3. `doc_architecture.template.json → gap_summary` is now the authoritative coverage map.
- Batch B — orchestration spine LANDED: `agent_registry.py` (C00–C06 derived from the document architecture: role, target_role, owning team, skills, human gate, allowed/forbidden actions), `orchestration.py` (`bodesign.c00.work_packet.v1` + `bodesign.c00.blocker_return.v1` persisted under `<folder>/_orchestration/` with `dispatch_work_packet`/`return_blocker`/`ingest_blocker`, deterministic IDs, append-only `log.jsonl`, fail-fast validation), and `mode_contracts.enter_c01_mode` (C00→C01, C01-I3 — PRD sections derived from `handoff_targets`, emits the C01 package, returns the next preference question; product-level decisions return as blockers). MCP: `bodesign_agent_registry` / `_dispatch_work_packet` / `_list_work_packets` / `_return_blocker` / `_list_blockers` / `_ingest_blocker` / `_enter_c01_mode`. +29 tests (173 total green).
- RB-2 DONE (Batch B): runtime-depended templates (`c00_prd.template.json`, `c00_prd.rubric.json`, `doc_architecture.template.json`) moved from `plans/` to `packages/workflow-core/bodesign_workflow_core/templates/`; loaders updated; `plans/.../TEMPLATES.md` records the move.

## Remaining

- Batch C: widen coverage so all 7 layers have minimal runtime — C04 layout constraint doc, C05 FW SW-spec sub-package scaffold (plan-builder-like), C06 test-plan assembler.
