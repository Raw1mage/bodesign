...
- Implement C03 mechanical-relevant constraint export from circuit design, install/configure OpenSCAD if real STL output is needed, then implement SketchUp import fallback, optional native SKP/STEP exporters, and tests for no guessed dimensions / no AI-only ME approval.
