...
- [~] **F-4 emit_doc→docxmcp binding — WON'T DO.** Would couple bodesign to the user's *private* docxmcp; other clients don't have it. bodesign stays self-contained: emits markdown (always), renders via local LibreOffice when present (else `skipped-no-soffice`), and a client's own docxmcp is driven by the agent — not by bodesign.
- [~] **F-5 spine dispatch to external-MCP layers — REVERTED.** The spine dispatches work packets uniformly and does not branch on backend (a client may have no external MCP; that knowledge belongs below the spine / to the agent). Removed `mcp_adapters.py`, the external_mcp dispatch branch, and the `mcp_caller` wiring. The `backend` field survives as native/worker **metadata** only. See `mcp_collaboration.md` "Overriding principle".
