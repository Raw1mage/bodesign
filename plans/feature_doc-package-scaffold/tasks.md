...
- [x] **E-5 Phase 3: lean core + delegate docs to docxmcp** — lean `Dockerfile.core` (no KiCad/LibreOffice/OpenSCAD/OCP); compose overlay builds core from it and forwards me+ee. **No bodesign-docs worker** — docx/pdf/pptx rendering is docxmcp's job (it already ships LibreOffice + the decompose/assemble system); bodesign emits markdown and `emit_doc` gates gracefully (`skipped-no-soffice`), then the agent renders via docxmcp. Every heavy bodesign dep now lives only in its worker; core image slim. (Removed the duplicate Dockerfile.docs.) docxmcp/drawmiat stay independent MCP endpoints; programmatic `emit_doc→docxmcp` delegation is optional future.
...
- [x] **F-5 spine dispatch to external-MCP layers (declarative)** — declarative `backend` ({native | worker:<group> | external_mcp:<server>+adapter}) added per layer in `doc_architecture.template.json`, surfaced via `agent_registry` as `role.backend` (validated). `c00_orchestration_tick` reads it with ONE branch on `backend.kind` (no per-layer if/else): native/worker unchanged; external_mcp invokes the declared adapter (`mcp_adapters.py`, pure mapping → `{server,tool,arguments}`) and calls the injected `mcp_caller` (`call_external_mcp_tool`, wired in the MCP handler — keeps workflow-core transport-free), recording the result or an `external-needed`/`blocked` blocker when unreachable/adapter-missing — never fabricated. All C00–C06 ship native/worker → no regression (verified). +4 tests (mock adapter + caller). The per-MCP adapter is the only hand-wired code (interface, not judgment).
