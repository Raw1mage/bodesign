...
- [x] **E-5 Phase 3: `bodesign-docs` + lean core** — `emit_doc` tagged `docs`; lean `Dockerfile.docs` (LibreOffice only) + lean `Dockerfile.core` (no KiCad/LibreOffice/OpenSCAD/OCP); compose overlay builds core from Dockerfile.core and forwards me+ee+docs. Every heavy dep now lives only in its worker; the core image is slim. docxmcp/drawmiat stay independent MCP endpoints (aggregation = optional future). +1 routing test.
