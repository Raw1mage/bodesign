...
def c00_orchestration_tick(folder: str | Path, *, auto_dispatch: bool = True) -> NextAction:
    ...
    if ready_undispatched:
        ...
        backend_kind = (role.backend or {}).get("kind", "native")  # metadata only (native|worker)
        ...
        # Uniform dispatch: create a work packet (C01 via its dedicated mode contract).
        # How the layer's work executes (native tool / bodesign worker / an external MCP
        # the client may have) is decided BELOW the spine — not here.
        if code == "C01":
